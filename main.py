...
my_joystick = Joystick(0.1,0.1,0.2,0.1)
my_camera = Camera()
my_command_center = CommandCenter()
my_screen = Screen()
state=States.IDLE
fourcc = cv2.VideoWriter_fourcc(*'XVID')
video = cv2.VideoWriter('Voo.avi', fourcc, 20.0, (1280, 720))
while state != States.EXIT:
    ...
    image = cv2.flip(image, -1) #Ivertendo a imagem
    cv2.imshow("video", image)
    cv2.waitKey(1)
    video.write(image) 
    my_camera.update_RGB_image()
    #my_camera.update_gray_image()
